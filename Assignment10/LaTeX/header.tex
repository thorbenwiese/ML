...
\documentclass[pdftex,a4paper,12pt,DIV15,BCOR20mm,parskip,numbers=noenddot]{scrbook}
	...
	\usepackage[latin1]{inputenc} % Zeichenkodierung laden für Unix/Windows-Systeme
	%\usepackage[babel,german=quotes]{csquotes} % Deutsche Eingabe von ü,ä,ö,ß erlauben	
  ...
  \usepackage{ae,aecompl}  % benötigt zur PDF-Erzeugung, vgl http://dsanta.users.ch/resources/type1.html
...
%	\usepackage{booktabs} % für tabellen: baut \midrule, \toprule und \bottomrule ein
%	\setlength{\tabcolsep}{30pt} % in Tabellen: Padding des Spalten nach links und rechts
	...
	% Benutzt man bestimmte Spaltendefinitionen (hier: Spaltentyp p mit 25% Breite) öfter, kann man diese hier auslagern und unter einem bestimmten Buchstaben (hier: A) speichern
%	\newcolumntype{A}{p{0.25\textwidth}} 

	\usepackage{nomencl} % Package für ein Abkürzungsverzeichnis
	%\renewcommand{\nomname}{Abkürzungsverzeichnis} % Deutsche Überschrift
	\setlength{\nomlabelwidth}{.20\hsize} % Breite eines Eintrags
	\renewcommand{\nomlabel}[1]{#1 \dotfill} % Auffüllen mit Punkten
	\setlength{\nomitemsep}{-2\parsep} % Zeilenabstand verkleinern
	\makenomenclature  % Datenbank des Abkürzungsverzeichnisses beim Kompilieren des Textes erzeugen

	% fuer Literaturverweise
	\usepackage[numbers,square]{natbib}
	%\bibpunct{[}{]}{;}{a}{}{;} % Natbib konfigurieren, so daß im text [Müller 2006] erscheint und keine runden Klammern o.ä.
	%\renewcommand{\cite}{\citep} % \cite zu \citep umwandeln, so dass im Text \cite{Mueller.2006} für Referenzen verwendet werden kann.
%	\usepackage{url} % damit im LitVZ auch so etwas wie 'Müller 2006: Vortrag zum Thema XY (Powerpoint-Präsentation)' steht; (Dateiendungen werden also textuell repräsentiert)

	%% Verhindert, dass im Literaturverzeichnis nach der Nennung von [Mueller 2006] die Seite umgebrochen wird und die Autoren + Buchtitel etc. auf der nächsten Seite landen
	%% Es wurde daher \noparagraphbreak in der natdin.bst bei der Ausgabe der Elemente ergänzt
	...
	% Definitionen werden mit Hilfe von 'dfn' eingeleitet und können somit zentral formatiert werden.
  ...
  \hypersetup
  {
  	...
  	pdfsubject   = {Universität Hamburg / Department Informatik / ITG / Diplomarbeit},
  	pdfauthor    = {Vorname Nachname},
  	pdfkeywords  = {Diplomarbeit, Universität Hamburg}, % Hier kann eine beliebige Liste von Schlüsselwörtern eingegeben werden, die z.B. Windows zur Indexierung/Suche benutzt
  	plainpages   = false 
  }
